...
afterAll(async () => {
  if (projectId) {
    try {
      execSync(
        `psql "${DB_URL}" -c "` +
        `DELETE FROM notification_channels WHERE agent_id IN (SELECT id FROM agents WHERE project_id = '${projectId}'); ` +
        `DELETE FROM subscriptions WHERE agent_id IN (SELECT id FROM agents WHERE project_id = '${projectId}'); ` +
