...
- `DELETE /agents/:id` — same `callerMayActOnAgent` gate as rotation.
- `PUT /agents/:id/heartbeat`, `GET /agents/:id` — **repo membership only, NOT caller-gated.** A peer in the same repo can forge another agent's heartbeat or read its record. Forged liveness is not cosmetic: `tryRulesRouting` only assigns to agents seen inside 10 minutes, so it can hold an offline agent in the routing pool.
