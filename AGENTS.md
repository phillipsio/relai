...
- `POST /agents/:id/tokens` — rotate; returns a new plaintext token. **Role-or-self gated**: the caller must be the target agent or an orchestrator in its repo, via `callerMayActOnAgent` (`lib/ownership.ts`). Repo membership alone used to be sufficient, which let any worker mint another agent's token — including an orchestrator's, which authors `shell` verify predicates and therefore reaches command execution in this process.
- `DELETE /tokens/:id` — revoke. Same `callerMayActOnAgent` gate: revocation carries the same authority as the rotation it undoes.
- `DELETE /agents/:id` — same `callerMayActOnAgent` gate as rotation. Six FKs point at `agents.id` with NO ACTION (`tasks.assigned_to`, `routing_log.assigned_to`, `invites.created_by`, `invites.accepted_agent_id`, `artifacts.owner_agent_id`, `artifact_versions.published_by_agent_id`), so the handler clears them all first, the way `DELETE /repos/:id` does. A task still in flight goes back to `pending` with `autoAssign`, because an assigned task with a null assignee is unreachable; terminal tasks keep their status and lose the pointer; `routing_log` rows are deleted, its column being NOT NULL. `repos.default_assignee` and `tasks.verify_reviewer_id` are cleared too. Those two are not foreign keys, so they raise nothing at delete time and break afterwards instead: a dangling `default_assignee` fails every later insert in that repo. They are not the only unconstrained agent-id columns — `messages.from_agent` and `messages.to_agent` are as well — but those two are attribution, nothing writes them back into a constrained column, and `from_agent` is `NOT NULL`, so a dangling id there is the correct outcome. The cascade runs in one transaction, as `DELETE /repos/:id` now does, so a scheduler tick that reassigns a task mid-cascade rolls the whole thing back.
- `PUT /agents/:id/heartbeat` — same `callerMayActOnAgent` gate. Only the agent can truthfully claim it is awake, and `tryRulesRouting` keeps agents seen inside 10 minutes, so a forged stamp held an offline peer in the routing pool until `STALLED_REAP_MS`.
- `GET /agents/:id` — **repo membership only, NOT caller-gated.** A peer in the same repo can read another agent's record, `repoPath` included.
- `GET /agents` — repo membership only, and **owner-scoped rather than repo-scoped**: it returns every agent whose repo shares this one's owner, not just the caller's own repo, so a peer in a sibling repo can be found and addressed. This is the read-shaped subset of cross-repo access: it discloses that an agent exists and whether it is awake, and nothing else — tasks, threads, messages and event delivery all stay repo-bound. Falls back to own-repo when `repos.ownerId` is null (the self-hosted default), because `owner_id = NULL` matches nothing and would otherwise hand back an empty directory, while a naive `IS NULL` match would pool every unowned repo on the instance. Exposed to agents as the `list_agents` MCP tool, which projects fields rather than returning the row: `repoPath` is a filesystem path on someone else's machine and is never included.
...
**Tasks**
- `POST /tasks`, `GET /tasks?repoId=&status=&assignedTo=&epicId=&archived=&limit=&clip=`, `GET /tasks/:id`, `PUT /tasks/:id` (`epicId=` filters an Epic's child Issues; archived tasks are excluded unless `archived=true`). `limit=`/`clip=true` are opt-in — omitted, the response is unbounded and unclipped exactly as before (the dashboard and `scripts/attention-check` depend on that shape); either way `meta.total`/`meta.returned` report the true count so a capped caller can tell what it didn't see. The MCP `list_all_tasks` tool always calls through with `limit=50`/`clip=true` by default, since an unfiltered call once came to 355k chars.
...
- `GET /tasks/:id/comments` — returns `{ threadId, comments }` for the Issue's comment thread, creating + linking it lazily on first access. It also **relinks** when the existing pointer names a thread this task cannot own, so the thread id a caller gets back is not always the one stored a moment earlier. `POST /tasks/:id/comments { body, type? }` posts a comment (caller identity, or `"human"` on the admin path). **An agent may read and comment on a task it created even in another repo** (`GET /tasks/:id`, and both comment routes): filing feedback is otherwise write-only, since `POST /relai-feedback` puts the task in the triage repo where the reporter's token has no access, so it could report a problem and never read or correct what it reported. Read-and-comment only — `PUT /tasks/:id`, archive, review and commit deliberately do not grant it. This is the unified-UI view of a task's linked thread; messages still flow through the `threads`/`messages` tables underneath.
- `POST /tasks/:id/commit` — orchestrator commits (or rejects) a `"proposed"` task. Body `{ decision: "commit"|"reject" (default "commit"), assignedTo? (agent id | "@auto" | omit→repo default), note?, + optional ratified edits: title/description/priority/domains/specialization/verify* }`. Caller must be an orchestrator agent **or** the deprecated admin-secret path, with one exception: the task's own `createdBy` may send `decision: "reject"` to withdraw it. Anyone else gets 403. Only a `"proposed"` task is committable (others → 409), and both arms re-check that in the write. On commit it resolves the effective assignee exactly like create, applies edits (re-validating any verify changes via the shared consistency + reviewer-existence checks), writes `metadata.commit = { committedBy, committedAt }`, transitions to `assigned`/`pending`, and emits `task.committed`. **The `@auto` arm writes `assignedTo: null`, not `undefined`** — drizzle omits an `undefined` key from `.set()`, so it used to leave whatever assignee was already on the row while reporting `autoAssign: true`, which is how a proposer that had written its own id onto the column ended up owning the committed task. Reject is additionally open to the task's own `createdBy` as a withdrawal. On reject it sets `cancelled`, records `metadata.proposal.rejectedBy/rejectedAt/note`, and emits `task.proposal_rejected` (notifying the proposer).
...
- `POST /subscriptions`, `GET /subscriptions?agentId=`, `DELETE /subscriptions/:id`. The two writes are `callerMayActOnAgent`-gated: deleting a peer's subscription silences its event and notification delivery, since `selectChannels` resolves channels through subscribers. **A subscription created through this route may not cross repos**: the target is resolved and its repo must match the subscribing agent's, and an unresolvable target is a 404. This matters because delivery (`resolveSubscribers`/`deliverableTo`) matches on `targetType`+`targetId` alone with no repo check, so a cross-repo row is a standing leak and the SSE payload carries the whole task record. `ensureSubscription()` is the deliberate exception, called server-side with ids the server chose (see `POST /relai-feedback`, which subscribes a reporter to the task it filed).
...
- `POST /notification-channels`, `PUT /notification-channels/:id`, `DELETE /notification-channels/:id` — all three are `callerMayActOnAgent`-gated on the channel's agent. Repo membership was enough until 2026-09-10, which meant any worker could PUT a peer's channel, repoint `config.url` at a host it controlled, and take back a regenerated HMAC secret in the response body. Reads needed nothing: `scopedAgentIds` already returns `[self]` for a per-agent caller.
- **The URL is an allowlist, not a format check** (`lib/outbound-url.ts`). https only, no `localhost` (trailing dot included), and no loopback, private, link-local, CGNAT, multicast or this-network address, literal or resolved. It applies to Slack channels too, and delivery re-resolves the hostname once per event, because a name that resolved publicly at insert can resolve privately later. **Delivery also passes `redirect: "manual"` and treats any 3xx as a failure**, which is not optional: the allowlist judges the stored URL, so one redirect would carry the POST, and the caller's own `config.headers`, anywhere it liked. Two windows stay open and are deliberate: Node's fetch resolves again after our check, so a rebind between the two still gets through (closing it needs an agent pinned to the checked address), and the re-resolution happens once per event rather than once per retry.
...
The blocked-task watcher detects human replies on threads referenced by `task.metadata.blockedThreadId` and resumes those tasks back to `assigned`. A reply only counts if its `authorKind` is `human` **and** it postdates `tasks.blockedAt`, which is stamped on every transition into `blocked` (including a re-block). Comparing against `createdAt` instead, as it once did, meant any message already on the thread resumed the task the instant it blocked. A watchable row with no `blockedAt` is skipped and warned about rather than resumed, so the watcher fails closed. **An agent's answer also resumes the task, but only from the agent that was asked**: the watcher finds the asker's own message on that thread naming a `toAgent`, and accepts a reply only from that agent. A self-addressed question grants nothing, and with no question at all no agent reply qualifies. Human answers outrank agent ones when both landed, and set `metadata.humanReply` as before; an agent answer sets `metadata.agentReply` (`body`, `fromAgent`, `at`) so the worker can weigh it as coming from a peer rather than a person. **A block does not wait forever.** Past `BLOCKED_OVERDUE_MS` (default 30 min, longer than its `PROPOSED_`/`REVIEW_` siblings because the counterparty may not be running at all) the watcher emits a one-time `task.blocked_overdue`, which is an owner-attention kind. What happens next depends on who was awaited: a task waiting on an **agent** is released to `assigned` with `metadata.blockedTimeout`, because there is somewhere to fall back to and the prompt tells the worker not to wait on the same thing twice; a task waiting on a **human** stays blocked and is only nudged, because the human *is* the fallback and proceeding without them defeats the point of blocking. A real answer always outranks a timeout. **A stalled task is reaped, not just flagged.** `detectStalls` stamps `stalledAt` and stops; `reapStalledTasks` acts on it `STALLED_REAP_MS` later. The disposition is **`pending` + `autoAssign`, not `assigned`**, which matters: `assigned` is still counted by the load-balancing query (`assigned` + `in_progress`), and because `detectStalls` only scans `in_progress`, an `assigned` row whose worker is dead is no longer stall-detectable either — it becomes invisible fiction instead of flagged fiction, and nothing ever picks it up because the agent it names is gone. Re-queueing hands the work back to routing, which picks an agent that is currently online, and clears `stalledAt` so the row can be detected again. Bounded by `STALLED_MAX_RELEASES` (`metadata.stallReleaseCount`), after which the task goes to `blocked` with `metadata.blockedReason` and **no `blockedThreadId`**, so `watchBlockedTasks` will not revive it — the same reasoning as `blockOverflowedTasks`. Emits `task.stall_released` per re-queue and a one-time `task.stall_exhausted` at the bound; only the latter is an owner-attention kind, since a release needs nobody. Note the repo scan that feeds the cycle counts **any** `in_progress` row: it previously filtered on `stalledAt IS NULL`, which excluded exactly the repos the reaper exists for.

The proposed-task watcher emits a one-time `task.proposed_overdue` (notifying the repo's orchestrators) when a worker's `proposed` task waits past `PROPOSED_OVERDUE_MS` without being committed, so proposals don't stall silently when no orchestrator is acting.
...
Twenty-four tools with model-agnostic descriptions (work with any MCP-compatible client):
...
**Owner mode (operator ingress).** Set `API_OWNER_TOKEN` (= the API's `SERVICE_ADMIN_TOKEN`) + `OWNER_ID=usr_…` instead of `API_SECRET`/`AGENT_ID`/`REPO_ID`, and the server exposes a separate **operator toolset** (`buildOperatorTools`) instead of the agent tools: `list_repos`, `list_agents`, `create_task`, `add_task_comment`, `report_relai_issue`, `list_attention`, `get_task`, `list_threads`, `get_thread_messages`, `reply_human`, `review_task`, `commit_proposal`, `assign_task`. **Owner mode has no agent identity**: there is no `agents` row, so it never appears in `list_agents`, holds no tasks, and cannot be sent anything (`POST /agents/:id/messages` 403s it, because a `dmKey` needs two agent ids). It **polls its own inbox**: every `OWNER_POLL_INTERVAL_MS` (default 60s) it diffs the owner's attention set across all repos and pushes an MCP logging notification per transition, which is how a Claude Desktop owner session can raise a push notification on a phone. Agent mode had this (`pollInbox`) and owner mode did not, so the console only ever saw what the operator thought to ask for. Two queries, because **stalled work is still `in_progress`**: its status looks healthy and only `stalledAt` gives it away, and `task.stalled` is not an owner-attention kind, so notification channels never fired for it either. On the first poll it emits one summary rather than one notice per item (opening a session otherwise fires dozens), and a task that leaves the attention set is forgotten so re-entry notifies again. The diff is a pure function in `owner-watch.ts`. `list_threads` + `get_thread_messages` exist because without them it could write to any thread but only ever *discover* the ones arriving as `metadata.blockedThreadId` on a blocked task, so `reply_human` on an unblock meant answering a question it could not read. These act across **all** the owner's repos (the client sends `X-Owner-Id`; the API scopes by `repos.ownerId`), addressing each resource by id — no `repoId` argument. `list_repos`/`list_agents` cover fleet discovery; `list_agents` computes an `online` boolean using the same 10-minute `lastSeenAt` window the routing scheduler uses. `create_task` accepts an agent name or ID for `assignedTo` (case-insensitive name resolution within the given repo). `assign_task` is the follow-up path for a task committed without an assignee (a `pending`/unassigned one, e.g. from `create_task`): it sets the assignee (agent name or ID, resolved within the task's repo) and moves the task to `assigned` via `PUT /tasks/:id` — `commit_proposal` only acts on `proposed` tasks, and `@auto` routing stays with `create_task`/`commit_proposal`. `reply_human` posts to a thread as `fromAgent="human"`, which is what the blocked-task watcher keys on to resume a stalled task, so it's the remote unblock primitive. Heartbeat/inbox polling are skipped (no single agent identity). See `docs/operator-ingress.md`.

**MCP SDK version**: pinned to `1.6.0`. From **1.24.0** every tool definition carries an `execution.taskSupport` field that Claude Code v2.x does not recognize, causing tools to be silently excluded from the deferred tool list even when the server is connected. The boundary was measured on 2026-09-02 by driving `tools/list` over raw stdio against each version: 1.22.0 and below emit `name`/`description`/`inputSchema` and nothing else, 1.24.0 and above add `execution: { taskSupport: "forbidden" }` to every tool. It was previously recorded here as 1.29+, which is wrong and matters, because the field's arrival coincides exactly with the first release that fixes an advisory.

**Three SDK advisories therefore have no version that both fixes them and keeps the pin**, and all three are unreachable in this deployment, which is why the pin stands rather than being a known-risk exception:

- *ReDoS in `UriTemplate`* (<1.25.2) needs a registered resource template with an exploded array pattern. This server registers 24 tools and zero resources.
- *DNS-rebinding protection off by default* (<1.24.0) is HTTP-transport only. `TRANSPORT` defaults to `stdio`, every agent-mode client here runs stdio, and the local operator ingress leaves `TRANSPORT` unset. But `docs/operator-ingress.md` instructs `TRANSPORT=http`, so anyone following that guide is in the affected configuration. `MCP_HOST` defaults to `127.0.0.1`, and that is exactly what makes rebinding the **primary** remote path rather than a refinement of anything: a loopback listener has no remote reach until a browser page rebinds its hostname to loopback and becomes same-origin.
- *Cross-client data leak via transport/server reuse* (>=1.10.0 <=1.25.3). The SDK version is not the point here, and saying "1.6.0 predates the range" was wrong in the way that matters. **relai implements the pattern itself, version-independently**: `packages/mcp-server/src/index.ts` builds one module-level `McpServer` and calls `server.connect(transport)` per `GET /sse`, and every SDK version assigns `this._transport = transport`, so the newest connection silently displaces the operator's. Upgrading the SDK does not fix our own code.

**The HTTP branch is broken and unauthenticated, and that outranks all three advisories.** Reproduced 2026-09-02: with the server run exactly as `operator-ingress.md` says, a second unauthenticated `GET /sse` received an owner attention notification naming a blocked task and its repo, while the operator's own stream received nothing. `GET /sse` checks no `Authorization`, `Origin` or `Host`. Separately `POST /messages` answers `res.writeHead(200).end()` and never calls `handlePostMessage`, so that deployment has never been able to serve a tool call at all: a client connects, then hangs on `initialize`. Fix the transport before revisiting the pin, because 1.24.0+ would buy `enableDnsRebindingProtection` and still leave the listener credential-free and the server shared.

Re-check reachability before treating this as settled: registering the first MCP **resource** template, or running any deployment with `TRANSPORT=http`, makes an advisory live and forces the upgrade-versus-tool-visibility trade for real. Verify a bump by listing tools over stdio and asserting the definitions carry no key beyond `name`, `description` and `inputSchema`. A passing typecheck and test suite prove nothing here, since both stayed green at 1.30.0 while the wire format changed.
...
**`capabilities: { logging: {} }` is mandatory if the server pushes notifications.** `sendLoggingMessage()` runs `assertNotificationCapability` first and throws `Server does not support logging` without it, and `new McpServer({name, version})` declares no capabilities. Both poll loops caught that throw and discarded it, so the notification path was dead from the day it was written and looked merely quiet: agent-mode inbox notifications never reached a client. Constructed via `createMcpServer()` in `create-server.ts` purely so the capability is unit-testable, since `index.ts` is an entrypoint with side effects. **A silent catch around a notification send is how this hid** — both now log to stderr.

**Reaching the owner: `scripts/owner-notify-relay.mjs`.** The notification subject is the **owner**, not a session, app or repo, and relai already models that: an owner-scoped `notification_channels` row fans out every attention transition across all that owner's repos via `event.repoId → repos.ownerId`. This is a localhost receiver for the `webhook` kind that turns those into macOS notifications with `osascript`, which is the whole delivery problem solved without Slack, a vendor account, or any session being open. Verifies the HMAC when `RELAY_SECRET` is set (the channel's `secret` column) and uses `execFile`, never a shell string, since a task title is arbitrary text. Event-driven, so unlike the polling script it has no interval and costs nothing idle. A launchd plist at `~/Library/LaunchAgents/com.relai.owner-notify-relay.plist` keeps it up across reboots.

**Reaching a human: `scripts/attention-check.mts`.** Prints one line per item that has newly entered an attention state (`blocked`/`pending_verification`/`proposed`/stalled across all the owner's repos) and **nothing at all when there is no news**, so any scheduler can notify on any output. State lives in `~/.relai-attention-seen.json`; a missing file means first run, which summarises rather than firing one notification per pre-existing item. It queries `in_progress` separately because stalled work still has that status and only `stalledAt` betrays it. It imports `diffAttention` from the MCP owner watcher rather than restating the rules, so "needs a human" cannot come to mean two different things. Credentials come from `~/relai-operator/.mcp.json` and are never printed. Any scheduler can drive it: a Claude Code cron, a Claude Desktop scheduled task, launchd, or plain cron.

**An MCP logging notification is not a push notification.** It reaches the client's log; it does not start a turn, so the model does not run and nothing acts. Waking an operator needs something scheduled: a `notification_channels` row (`kind: "slack"` posts a readable summary out of band), a Claude Code session with a cron, or a headless agent. A Claude Desktop session cannot be woken by relai.
...
**Tool slot limit**: Claude Code exposes a finite number of MCP tools per session. If you have many MCP servers, the relai tools may not surface. Disable unused MCP servers or move relai to `~/.claude.json` to prioritize it. The tools are working correctly if `/mcp` shows relai as connected with twenty-four tools.
...
**Wire this repo's own SessionStart watcher hook, same as every consumer repo.** Fixed 2026-08-25 (`task_zxbZMjVHyctb4YZjy5KwO`): `.claude/settings.json` points SessionStart at `scripts/relai-watch-hook.sh`, which is what makes an interactive session wake on relai events rather than noticing them only when a human looks. Every consumer repo (`functionize-mcp-go`, `front-end-app-v2`) had this; relai's own checkout did not, so its orchestrator was the one agent in the fleet that never woke — and it then inferred from its own silence that *no* agent wakes, writing that wrong conclusion into two task comments before a live test disproved it. **Do not diagnose "is anything driving this agent" from `launchctl list`**: the watcher is a backgrounded child of the interactive session and never appears there. Check the process table for `relai-stream-wait.sh`, or the agent's self agent-target row in `subscriptions`. Together with the `npx` bug above, that is twice this repo has been the least well-configured relai client, because consumer repos get configured deliberately and this one is assumed to already work.
...
CI (`.github/workflows/ci.yml`) runs `pnpm typecheck` then the two halves of `pnpm test` on every push to `main` and on every pull request, against a `postgres:16` service. It creates `relai_test`, applies migrations, and sets `TEST_DATABASE_URL` (port 5432 in CI, where nothing else competes for it, rather than the 5433 used locally). It **also migrates the plain `relai` database and passes it as `DATABASE_URL` to the shell step**, because the join e2e boots a real API against a real database and defaults to that one. **The test run is split into `pnpm exec vitest run` and `pnpm test:shell` for exactly that reason**: only `packages/api` pins its own `DATABASE_URL` (through `test.env` in its vitest config), so giving the vitest step a live pointer at a database nothing truncates would mean the next project to open a connection quietly writes to `relai`. The `globalSetup` that TRUNCATEs reads `TEST_DATABASE_URL`, never `DATABASE_URL`, so no value here can aim a truncate at the wrong database. Before CI existed the only gate was a machine-local pre-push hook that any `SKIP_PR_REVIEW=1` bypassed.

**`gh` can read this repo's Actions even though it cannot open PRs against it.** `gh run list`, `gh run view <id> --log` and `--log-failed` all work; only the PR routes are blocked by the work-account restriction below. That distinction matters, because assuming all `gh` access was blocked is why eleven consecutive red runs went unexamined between 2026-09-10 and 2026-09-17. Two different causes hid in there: a stale `pnpm-lock.yaml` against `packages/cli/package.json`, and then the join e2e failing on `npx tsx` resolving nothing on a runner.

**The shell suite is part of the gate, not a local extra.** `scripts/run-shell-tests.sh` runs every `scripts/tests/*.test.sh`: `watcher.test.sh` covers the wake-path scripts (`relai-watch.sh`, `relai-stream-wait.sh`, the SessionStart hook) that no vitest suite reaches, and `join-e2e.test.sh` wraps `packages/cli/scripts/test-join-e2e.sh`, which drives `relai join` end to end against a real API in a throwaway repo with `HOME` redirected. That script has to work on both a laptop and a runner, which are differently equipped: it resolves `tsx` from each package's own `node_modules/.bin` (never `npx`, which resolves nothing on a runner since no root package declares tsx), reaches Postgres through `psql` when present and `docker exec relai-postgres-1` otherwise, and skips its `lsof` port check rather than failing open when `lsof` is absent.

Tests use vitest. Test files live alongside source as `*.test.ts`.

**The `packages/api` suite runs against a dedicated `relai_test` database** (same Postgres container/port 5433, same `relai`/`relai` role — no new user needed), never the dev DB. `packages/api/vitest.config.ts` sets `test.env.DATABASE_URL` to `relai_test`, which overrides every test file's own `DATABASE_URL ?? "...relai"` fallback, and a `globalSetup` (`packages/api/src/test/global-setup.ts`) truncates every table before each run. This makes DB hygiene structural rather than per-test discipline: a test that forgets cleanup can only pollute state within its own run, and it can never touch what the local dashboard shows. **The corollary: never run two suites at once against it.** `globalSetup` truncates every table at the start of each run, so a second `pnpm test` pulls the floor out from under the first and both fail on unrelated assertions. Reproduced 2026-09-10: two concurrent runs, both red, different tests each. This matters here more than in most repos, because several agents may reach for `pnpm test` at the same time; if a run fails in a way that looks like a race, check whether anyone else is running the suite before debugging the test. (This repo hit the discipline-based failure mode twice — 2026-05-06 and 2026-06-26 — before this fix landed.) One-time setup, and again after any schema change: see "Dev setup" below. Schema changes must be applied to **both** `relai` and `relai_test` via `db:migrate`.
...
- `packages/api/src/routes/task-thread-boundary.test.ts` — a task's comment thread cannot be re-pointed at another project's thread or a DM, from either direction (the write is gone, the read relinks), and a relink leaves an audit record a client cannot forge
...
| `STALLED_REAP_MS` | `3600000` | How long a task sits with `stalledAt` stamped before the reaper hands its work back. On **top of** `STALL_THRESHOLD_MS`, so the real delay is the sum (~5h by default). |
| `STALLED_MAX_RELEASES` | `2` | How many times a stalled task may be re-queued before it is blocked for a human instead. Re-queueing is a retry; without a bound an un-completable task loops forever. |
...
| `OWNER_POLL_INTERVAL_MS` | `60000` | Owner-mode MCP attention poll interval. Each transition into `blocked`/`pending_verification`/`proposed`/stalled pushes one MCP logging notification. |
...
| `TASKS_MAX_LIMIT` | `200` | Ceiling `GET /tasks?limit=` clamps to, including a present-but-unparseable value (`limit=abc`) — never falls through to unbounded. |
| `TASKS_DESC_CHARS` | `500` | Task descriptions clipped to this when `GET /tasks?clip=true`. |
| `TASKS_META_CHARS` | `800` | Task `metadata` above this collapses to its key list when `GET /tasks?clip=true`. |
| `LIST_TASKS_LIMIT` | `50` | Default `limit` the MCP `list_all_tasks` tool passes to `GET /tasks` when the caller doesn't specify one. |
...
| `RELAI_REPO_PATH` | — | MCP agent-mode: explicit, exclusive override for the repo guard's working directory. Only consulted when `process.cwd()` isn't a git repo at all (cwd's own verdict is otherwise authoritative); requires a real clone with a matching origin, not just any directory — a bare `git init` does not satisfy it. For a client with no cwd control (Cursor), set this instead of the old `sh -c 'cd … && exec …'` wrapper. |
