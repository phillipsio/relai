...
- **The URL is an allowlist, not a format check** (`lib/outbound-url.ts`). https only, no `localhost`, and no loopback, private, link-local, CGNAT, multicast or this-network address, literal or resolved. It applies to Slack channels too, and delivery re-resolves the hostname because a name that resolved publicly at insert can resolve privately later. Node's fetch resolves again after that check, so a rebind in between still gets through; closing it needs an agent pinned to the checked address. The API host issues these requests, so this is the difference between a webhook and an SSRF primitive against the metadata endpoint.
