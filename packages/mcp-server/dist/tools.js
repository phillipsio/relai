...
function resolveAgentRef(candidates, ref, scope) {
...
function buildTools(client, agentId, repoId) {
    return [
        ...
        {
            name: "list_agents",
            description: "List the agents you can see, so you can address one. Covers every repo sharing this " +
        ...
        },
        {
            ...
            inputSchema: zod_1.z.object({
                threadId: zod_1.z
                    .string()
                    .optional()
                    .describe("The thread to post to. Use list_threads to find or create one. Omit it (with toAgent set) " +
                ...
                toAgent: zod_1.z
                    .string()
                    .optional()
                    .describe("Agent ID to address directly. Omit to send to the orchestrator. Required when threadId " +
                    "is omitted."),
                metadata: zod_1.z
                    .record(zod_1.z.unknown())
                    .optional()
            ...
            handler: async (input) => {
                if (!input.threadId) {
                    ...
                    return { content: [{ type: "text", text: JSON.stringify(dm, null, 2) }] };
                }
        ...
        {
            name: "archive_task",
            description: "Archive a finished task so it drops out of the default task lists and your session_start " +
                ...
                "the default views.",
            inputSchema: zod_1.z.object({
                taskId: zod_1.z.string().describe("The completed or cancelled task ID to archive."),
            }),
            handler: async (input) => {
        ...
        {
            name: "archive_thread",
            description: "Archive a concluded thread (planning OR operational) so it drops out of the default thread " +
            ...
            inputSchema: zod_1.z.object({
                threadId: zod_1.z.string().describe("The concluded thread ID to archive."),
            }),
            handler: async (input) => {
        ...
        },
        {
            ...
            handler: async (input) => {
                ...
                const notShown = describeCap("tasks", total, tasks.length, "a status filter or a higher limit");
                return {
                    content: [{
                            type: "text",
                            text: tasks.length === 0
        ...
        {
            name: "commit_task",
            description: "Act on a proposed task (a task in status 'proposed'). Committing is orchestrator-only; a task's own " +
            ...
            inputSchema: zod_1.z.object({
                taskId: zod_1.z.string().describe("The ID of the proposed task to commit or reject."),
                ...
                title: zod_1.z.string().min(1).optional().describe("Ratified title edit."),
                description: zod_1.z.string().min(1).optional().describe("Ratified description edit."),
                priority: zod_1.z.enum(["low", "normal", "high", "urgent"]).optional().describe("Ratified priority."),
                domains: zod_1.z.array(zod_1.z.string()).optional().describe("Ratified domain tags."),
                specialization: zod_1.z.string().optional().describe("Ratified specialization."),
                verifyKind: zod_1.z.enum(["shell", "file_exists", "thread_concluded", "reviewer_agent"]).optional(),
                verifyReviewerId: zod_1.z.string().optional(),
                verifyThreadId: zod_1.z.string().optional(),
                verifyPath: zod_1.z.string().optional(),
                verifyCommand: zod_1.z.string().optional(),
                verifyCwd: zod_1.z.string().optional(),
                verifyTimeoutMs: zod_1.z.number().int().optional(),
            }),
            handler: async (input) => {
                ...
                const body = { decision: decision ?? "commit" };
        ...
        {
            name: "assign_task",
            description: "Give an existing task an assignee, or move it to a different agent. This is the only way " +
            ...
            inputSchema: zod_1.z.object({
                taskId: zod_1.z.string().describe("The task to assign."),
                assignedTo: zod_1.z.string().describe("Agent ID (agent_*) or agent name in this project. '@auto' is not accepted here."),
                status: zod_1.z
...
function buildOperatorTools(client, ownerId) {
    return [
        ...
        {
            name: "list_threads",
            description: "List conversation threads across your repos (or one repo with repoId). Returns id, title, " +
            ...
            inputSchema: zod_1.z.object({
                repoId: zod_1.z.string().optional().describe("Limit to one repo. Omit for all your repos."),
                type: zod_1.z.string().optional().describe("Filter by type, e.g. 'plan' for Epics. Omit for all."),
            }),
            handler: async (input) => {
        ...
        {
            name: "get_thread_messages",
            description: "Read a thread's messages, newest-last. Call this before reply_human on a blocked task: the " +
            ...
            inputSchema: zod_1.z.object({
                threadId: zod_1.z.string().describe("Thread to read. From list_attention (metadata.blockedThreadId), list_threads, or get_task."),
                limit: zod_1.z.number().optional().describe("How many of the most recent messages to return (default 20)."),
            }),
            handler: async (input) => {
                const limit = input.limit ?? 20;
                const all = (await client.getMessages(input.threadId));
