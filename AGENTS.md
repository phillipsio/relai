...
**MCP SDK version**: pinned to `1.6.0`. From **1.24.0** every tool definition carries an `execution.taskSupport` field that Claude Code v2.x does not recognize, causing tools to be silently excluded from the deferred tool list even when the server is connected. The boundary was measured on 2026-09-02 by driving `tools/list` over raw stdio against each version: 1.22.0 and below emit `name`/`description`/`inputSchema` and nothing else, 1.24.0 and above add `execution: { taskSupport: "forbidden" }` to all 23 tools. It was previously recorded here as 1.29+, which is wrong and matters, because the field's arrival coincides exactly with the first release that fixes an advisory.
...
- *ReDoS in `UriTemplate`* (<1.25.2) needs a registered resource template with an exploded array pattern. This server registers 23 tools and zero resources.
