...
import type { WebApiClient, RepoRow } from "../lib/api";
  ...
  return (
    <div className="rounded-lg border border-zinc-700 bg-zinc-900 p-4 space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-semibold text-zinc-200">Repo created</h2>
...
function CreateRepoForm({ api, onCreated }: { api: WebApiClient; onCreated: (p: RepoRow) => void }) {
  ...
  if (!open) {
    return (
      <Button size="sm" onClick={() => setOpen(true)}>+ New repo</Button>
    );
  }
  ...
  return (
    ...
    >
      <h2 className="text-sm font-semibold text-zinc-200">New repo</h2>
      ...
      <div className="flex gap-2 justify-end">
        ...
        <Button type="submit" size="sm" disabled={create.isPending || !name.trim()}>
          {create.isPending ? "Creating…" : "Create repo"}
        </Button>
      </div>
    </form>
  );
...
export function Repos({
  ...
  const qc = useQueryClient();
  const [newRepo, setNewRepo] = useState<RepoRow | null>(null);
  ...
  const del = useMutation({
    ...
    onSuccess: (_, id) => {
      qc.invalidateQueries({ queryKey: ["repos"] });
      if (id === api.repoId) onSwitch("");
    },
  });

  function handleSwitch(p: RepoRow) {
    saveConfig({ apiUrl: api.apiUrl, apiSecret: api.apiSecret, repoId: p.id });
    onSwitch(p.id);
  }

  function handleCreated(p: RepoRow) {
    handleSwitch(p);
    setNewRepo(p);
  }
  ...
  if (newRepo) {
    return (
      <div className="space-y-4">
        <h1 className="text-xl font-semibold font-mono">Repos</h1>
        <PostCreationGuide project={newRepo} onDone={() => setNewRepo(null)} />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold font-mono">Repos</h1>
        <CreateRepoForm api={api} onCreated={handleCreated} />
      </div>

      {repos.isLoading && <p className="text-sm text-zinc-500">Loading…</p>}

      <div className="space-y-2">
        {(repos.data ?? []).map((p) => (
          <Card key={p.id} className={p.id === api.repoId ? "border-zinc-600" : ""}>
