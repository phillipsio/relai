...
- *DNS-rebinding protection off by default* (<1.24.0) is HTTP-transport only. `TRANSPORT` defaults to `stdio`, and every agent-mode client here runs stdio. **This is the weak leg of the argument.** `docs/operator-ingress.md` documents the owner-mode deployment with `TRANSPORT=http`, so anyone following that guide is running the affected configuration, and the advisory is live for them. It is not live for the local operator ingress, whose `.mcp.json` leaves `TRANSPORT` unset, which is what was actually measured on 2026-09-02. Note the ordering of risk: that same doc already warns its listener "accepts any `GET /sse` / `POST /messages` with no credential check" while holding `API_OWNER_TOKEN`, so an unauthenticated transport is the larger problem and DNS rebinding is a refinement of it.
- *Cross-client data leak via transport/server reuse* (>=1.10.0 <=1.25.3) is likewise HTTP-only, and 1.6.0 predates the affected range entirely.
